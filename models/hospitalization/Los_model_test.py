...
def plot_pr_curve(y_test, y_score, name):
    ...
    plt.savefig('src/output/PR_curve_v1_{}.pdf'.format(feat_eng))
...
def plot_roc_curve(y_test, y_score, name = ''):
    ...
    plt.savefig('src/output/ROC_curve_v1_{}.pdf'.format(feat_eng))
...
df_merged_shu = pd.read_csv('src/int_data/feat_eng/merged_data_{}.csv'.format(feat_eng)) # shuffled dataframe
start = 2
...
clf_rf_train = RandomForestClassifier(n_estimators=100, min_samples_leaf=2, class_weight='balanced',
                                      max_features='auto',
...
plt.savefig('src/output/imp_features.pdf', dpi=900)


# SVM
clf_svm = SVC(kernel='linear')
param_grid_svm = {
    "C": [ 0.001, 0.01, 0.1, 1, 2, 5, 10, 50, 100, 1000],
